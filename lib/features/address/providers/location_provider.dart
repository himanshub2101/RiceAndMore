...
class LocationProvider with ChangeNotifier {
 ...
 void getCurrentLocation(BuildContext context, bool fromAddress, {GoogleMapController? mapController, bool isManual = false}) async {
  _loading = true;
  notifyListeners();
  ...
  if(fromAddress) {
    updateAutomaticLocation(myPosition, await getAddressFromGeocode(LatLng(myPosition.latitude, myPosition.longitude)));
  } else {
    updateManualLocation(myPosition, '');
  }
...
// Method to update location when retrieved automatically
void updateAutomaticLocation(Position position, String address) {
  _position = position;
  _address = address;
  // Enable save button when valid location data is available
  _buttonDisabled = false;
  notifyListeners();
}

// Method to update location when entered manually
void updateManualLocation(Position position, String address) {
  _position = position;
  _address = address;
  // Enable save button when valid location data is available
  _buttonDisabled = false;
...
// Method to handle saving location
void saveLocation() {
  if (_position.latitude != 0 && _position.longitude != 0 && _address != null && _address!.isNotEmpty) {
    // Save location using _position and _address variables
    // Example: save to SharedPreferences or send to server
    // Reset button state after saving
    _buttonDisabled = true;
    notifyListeners();
  } else {
    // Handle case where location data is incomplete
    // Show error message or prompt user to enter valid location
  }
}
